...
/* ===========================================================================
   TRDOS 386 Port - Independent Inline ASM Early Boot Test Block (sysmsg)
   =========================================================================== */
#ifdef TCC_TARGET_I386
    /* Direct TRDOS 386 sysmsg call to print a string with color support */
    /* This completely bypasses MinGW headers and write.s object files */
    __asm__ __volatile__ (
        "movl %0, %%ebx\n\t"        /* EBX = Message pointer address */
        "movl $36, %%ecx\n\t"       /* ECX = Message length (36 bytes) */
        "movb $0x07, %%dl\n\t"      /* DL  = Color attribute (0x07 = Light Gray/White) */
        "movl $35, %%eax\n\t"       /* EAX = 35 (TRDOS 386 sysmsg) */
        "int $0x40\n\t"             /* Kernel system call to print string */

        /* Direct TRDOS sys_exit to terminate process safely right here */
        "movl $0, %%ebx\n\t"        /* Exit Code = 0 */
        "movl $1, %%eax\n\t"        /* EAX = 1 (TRDOS sys_exit) */
        "int $0x40\n\t"             /* Kernel system call to exit */
        :
        : "g" ("TRDOS 386: TCC main entry verified!\r\n")
        : "eax", "ebx", "ecx", "edx"
    );
#endif
/* =========================================================================== */
