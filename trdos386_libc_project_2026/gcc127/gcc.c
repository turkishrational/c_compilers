...
execute ()
{
  ...
  /* Windows Bölümü */
  if (win)
    status = _spawnv (_P_WAIT, temp, argbuf);
  else
    status = _spawnvp (_P_WAIT, argbuf[0], argbuf);

  if (status < 0)
    pfatal_with_name (argbuf[0]);

  /* Windows'ta status zaten çýkýþ kodudur, maskelemeye gerek yok */
  if (status >= MIN_FATAL_STATUS)
    return -1;
  if (status != 0)
    return -1; /* Genel hata durumu */
    
  return 0; 
#else
  pid = fork ();
  if (pid < 0) pfatal_with_name ("fork");
  if (pid == 0)
    {
      if (win) execv (temp, argbuf);
      else execvp (argbuf[0], argbuf);
