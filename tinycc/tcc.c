...
/* 25/6/2026 */ 
unsigned int trdos_code_counter = 0;
...
/* 25/6/2026 - Google AI */
/* TRDOS SAF KARAKTER OKUMA REÇETESÝ */
...
int trdos_getc(void) {
    ...
    /* file global deðiþkenini ham fd olarak kullanýp doðrudan diskten 1 byte çekiyoruz */

    if (read((int)file, &ch, 1) <= 0) {

        return -1; /* EOF (End of File) standardý */
...
/* Eski kilitlenen makrolarý bu yeni saf fonksiyona mühürleyin: */

#undef inp

#define inp() trdos_getc()
...
void g(c)
{
    *(char *)ind++ = c;
    /* 25/6/2026 */
    /* RAM'e basýlan her saf flat makine kodu baytýný milimetrik sayýyoruz */
    trdos_code_counter++;
}
...
int main(int c, char **v)
{
    ...
    printf("-> [TRDOS]: Opening source file...\n");
    file = fopen(*v, "r");
    if (!file) {
    ...
    printf("-> [TRDOS]: Source opened successfully (FD=%d). Allocating tables...\n", (int)file);
    ...
    printf("-> [TRDOS]: Tables established. Executing next()...\n");
    next();
    
    printf("-> [TRDOS]: Lexer next() passed. Executing decl()...\n");
    decl(VT_CONST);
    
    printf("-> [TRDOS]: Compilation done. Jumping to main dynamic execution pointer...\n");
    
    /* JIT/RAM çalýþtýrma döngüsünü tamamen iptal edip DOÐRUDAN DISKE YAZMA katmanýna geçiyoruz */
#if 1  
    { 
        void *f;
        int code_bytes; /* Üretilen saf makine kodunun toplam boyutu */

        /* Heterojen pointer aritmetiðini tamamen bypass edip saf sayaç deðerini alýyoruz */

        code_bytes = (int)trdos_code_counter;

        /* Saf C standardýnda harici write fonksiyonu prototipi */

        extern int write(int fd, const void *buf, unsigned int count);
        
        /* 1. Adým: Saf assembler _fopen tetikleniyor (sys_creat üzerinden +3 alacak) */
        f = fopen("test.bin", "w");
        if (!f) {
            printf("Error: Cannot create output file 'test.bin'!\n");
            return 1;
        }

        /* 2. ADIM: Eðer hesaplanan boyut 0'dan büyükse gerçek flat makine kodunu diske mühürle */
        if (code_bytes > 0 & code_bytes < TEXT_SIZE) {
            /* prog tam sayýsýný char* pointer'a cast ederek saf kod bloklarýný yazýyoruz */
            write((int)f, (const void *)prog, code_bytes);
            printf("-> [TRDOS SUCCESS]: Real flat binary code (%d bytes) written cleanly to test.bin!\n");
        } else {
            /* Emniyet kilidi: Her ihtimale karþý ham bellek tabanýndan blok bas */
            write((int)f, (const void *)prog, 64);
            printf("-> [TRDOS WARNING]: Calculation error! Forced 64 bytes written to test.bin!\n");
        }
        
        /* 3. Adým: Saf assembler _fclose tetikleniyor (-3 recovery yapacak) */
        fclose(f);
        
        printf("-> [TRDOS SUCCESS]: 'test.bin' (%d bytes) written physically to disk!\n", code_bytes);
        
        /* TRDOS Zorlu Tahliye (Forced Exit) kalkaný tetikleniyor, prompt'a güvenli dönüþ */
