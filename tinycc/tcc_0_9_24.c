                    ...
                    if (v != 0 || (type1.t & VT_BTYPE) == VT_STRUCT) {
                        ...
                        if (lbit_pos == 0) {
                            if (a == TOK_STRUCT) {
                                c = (c + align - 1) & -align;
                                offset = c;
                                // if (size > 0) /* 03/08/2026 - TCC 0.9.24 */
                                c += size;
                            } else {
                                offset = 0;
                                if (size > c)
    ...
    if (tok == TOK_IF) {
        ...
        if (tok != ';') {
            ...
            if ((func_vt.t & VT_BTYPE) == VT_STRUCT) {
                CType type;
                /* Structure return: Copy values straight into the implicit first pointer argument block allocation */
                type = func_vt;
                mk_pointer(&type);
...
static int tcc_preprocess(TCCState *s1)
{
    ...
    /* Standart çıktı descriptor değeri C katmanında 1'dir (write.s .L_write_stdio dalını tetikler) */
    out_fd = 1;
    write_to_file = 0;

    /* Global static olan dosya adı string'ini (outfile) kontrol edip saf assembler fopen'ı tetikliyoruz */
    if (outfile && outfile[0] != '\0') {
        // int file_handle = (int)fopen(outfile, "w");
        /* 02/08/2026 */
        // out_fd = trdos_fopen(outfile, "w");
        ...
        // out_fd = file_handle; /* Çıktı kanalını bu yeni dosyaya mühürle */
        write_to_file = 1;
    }
    ...
    /* 06/08/2026 */
    // parse_flags = PARSE_FLAG_ASM_COMMENTS | PARSE_FLAG_TOK_NUM | PARSE_FLAG_PREPROCESS | PARSE_FLAG_LINEFEED;

    last_is_space = 1; /* Yeni satır/başlangıç temiz başlar */
    ...
    while (tok != TOK_EOF) {
        ...
        if (tok_str && tok_str[0] != '\0') {
            ...
            if (tok == TOK_LINEFEED) {
               ...
               /* Kelimelerin arka arkaya yapışıp ezilmesini engellemek için,
                  bir önceki token boşluk değilse araya tam 1 baytlık boşluk basıyoruz.
                 f_putc fonksiyonunu bypass edip, statik bir boşluk karakteri adresini doğrudan yazıyoruz!
                */
               if (!last_is_space && tok_str[0] != '(' && tok_str[0] != ')' && tok_str[0] != ';') {
            ...
            /* =========================================================================
               CRITICAL BYPASS: f_puts katmanını tamamen sildik.
               Hizasız token dizesinin (tok_str) uzunluğunu doğrudan yerel döngüyle,
               hiçbir fonksiyon çağrısı yapmadan (stack'e dokunmadan) buracıkta hesaplıyoruz.
               ========================================================================= */
            len = 0;
            while (tok_str[len]) {
                len++;
            ...
            /* Ham write fonksiyonu, dizeyi tam uzunluğu (len) kadar kernel disk tamponuna basar */
            if (len > 0) {
               if (out_fd > 2) {
                  write(out_fd, tok_str, len);
               /* 04/08/2026 - Google AI */
               } else {
            ...
            /* 04/08/2026 */
            last_is_space = 0; /* Karakter basıldığı an boşluk zırhı kalkar */
        }
        next();
    }

    /* Eğer bir dosya açtıysak, +3 zırhlı handle'ı assembler tabanlı fclose ile kapat */
    if (write_to_file) {
        // fclose((void *)out_fd);
        /* 02/08/2026 */
        // trdos_fclose(out_fd);
